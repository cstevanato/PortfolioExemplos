package com.example.portfolio.exemplos.features.exampleDataRetry.retryWhenArrow
...
interface PostRepository {
    fun fetchPosts(): Flow<Either<NetworkError, List<Post>>>
    fun fetchPostById(id: Int): Flow<Either<NetworkError, Post>>
}
...
class PostRepositoryImpl(
    ...
    override fun fetchPosts() = flow<Either<NetworkError, List<Post>>> {
        val data = api.getPosts().map { it.toDomain() }
        emit(data.right())
    }
        .retryWhen { cause, attempt ->
            // attempt é 0-based e representa quantos retries já ocorreram.
            if (attempt >= maxRetries) return@retryWhen false

            val shouldRetry = shouldRetry(cause)
            if (shouldRetry) {
                // Backoff simples opcional (exponencial leve)
        ...
        .catch { e ->
            emit(mapThrowable(e).left())
        }
        .flowOn(Dispatchers.IO)

    override fun fetchPostById(id: Int) = flow<Either<NetworkError, Post>> {
    ...
    /**
     * Mapeia Throwable -> NetworkError para emissão final (Left)
     */
...
sealed class NetworkError {
    ...
    data class ClientError(val statusCode: Int, val message: String?) : NetworkError()
